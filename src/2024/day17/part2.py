...
for line in lines:
  ...
  if key.startswith("Register "):
    ...
    startingStores[register] = int(value.strip())
  elif key == "Program":
...
while output != program:
  stores = startingStores.copy()
  currentA += 1
  stores["A"] = currentA
  output = []
  pointerLocation = 0

  while pointerLocation < len(program) and program[:len(output)] == output:
    opcode, operand = program[pointerLocation:pointerLocation+2]
    processInstruction(opcode, operand)
    pointerLocation += 2
  if output != program:
    matchLength = len(output) - 1
    while matchLength > 0:
      if matchLength not in matchIndexes:
        matchIndexes[matchLength] = [currentA]
        print(matchLength, ":", matchIndexes[matchLength])
        sys.stdout.flush()

      elif len(matchIndexes[matchLength]) < 20:
        matchIndexes[matchLength].append(currentA)
        print(matchLength, ":", matchIndexes[matchLength])
        sys.stdout.flush()
      matchLength -= 1


print("Answer", currentA)
