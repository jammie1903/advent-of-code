let [seeds, ...maps] = $("pre").innerText.split("\n\n").filter(Boolean)
.map(c => c.split("\n").filter(Boolean))
seeds = seeds[0].substring(seeds[0].indexOf(":") + 1).trim().split(" ").map(Number)
maps = maps.map(map => {
...
console.log("result", seeds.map(processor).reduce((a, b) => a < b ? a : b))
